...
if (isset($_POST['hitung'])) {
        ...
        foreach ($criteria as $criterion) {
            ...
            if ($result_sub_criteria->num_rows > 0) {
                ...
                while ($row_sub_criteria = $result_sub_criteria->fetch_assoc()) {
                    ...
                    // Simpan nilai likelihood ke database atau lakukan tindakan lainnya
                    $sql_save_likelihood = "
                        INSERT INTO likelihood (criteria_id, sub_criteria_id, likelihood_naik, likelihood_tidak_naik)
                        VALUES ($criteria_id, $sub_criteria_id, $likelihood_naik, $likelihood_tidak_naik)
                    ";
                    $conn->query($sql_save_likelihood);
                }
            }
        }
    } else {
    ...
    // Fungsi untuk melakukan klasifikasi Naive Bayes dan menyimpan hasil
    function hitungNaiveBayes($conn, $karyawan_data, $criteria, $prior, $likelihood, $total_data) {
        // Klasifikasi setiap karyawan
        foreach ($karyawan_data as $karyawan) {
            $posterior = [];
            foreach ($prior as $kelas => $prob_prior) {
                $posterior[$kelas] = $prob_prior;
                foreach ($criteria as $criterion) {
                    $sql_value = "SELECT value FROM karyawan_kriteria 
                                WHERE karyawan_id = {$karyawan['id']} 
                                AND criteria_id = {$criterion['id']}";
                    $result_value = $conn->query($sql_value);
                    if (!$result_value) {
                        die("Error getting criteria value: " . $conn->error);
                    }
                    $value_row = $result_value->fetch_assoc();
                    if ($value_row) {
                        $value = $value_row['value'];
                        if (isset($likelihood[$criterion['id']][$kelas][$value])) {
                            $posterior[$kelas] *= $likelihood[$criterion['id']][$kelas][$value];
                        } else {
                            // Laplace smoothing jika tidak ada data
                            if (isset($likelihood[$criterion['id']]) && is_array($likelihood[$criterion['id']])) {
                                $posterior[$kelas] *= 1 / ($total_data + count($likelihood[$criterion['id']]));
                            } else {
                                $posterior[$kelas] *= 1 / ($total_data + 1); // Sesuaikan jika likelihood criterion tidak diset
                            }
                        }
                    }
                }
            ...
            if (!empty($posterior)) {
                $hasil_klasifikasi = array_keys($posterior, max($posterior))[0];
            } else {
                $hasil_klasifikasi = "Tidak dapat diklasifikasikan"; // atau nilai default lainnya
            }

            // Cek apakah hasil klasifikasi sudah ada
            $sql_check = "SELECT * FROM hasil_klasifikasi WHERE karyawan_id = {$karyawan['id']}";
            $result_check = $conn->query($sql_check);
            
            if ($result_check->num_rows == 0) {
                // Simpan hasil klasifikasi
                $sql_insert = "INSERT INTO hasil_klasifikasi (karyawan_id, hasil, status, tanggal) VALUES ({$karyawan['id']}, '$hasil_klasifikasi', '$hasil_klasifikasi', NOW())";
                if (!$conn->query($sql_insert)) {
                    die("Error inserting result: " . $conn->error);
                }
            }

            // Cek apakah hasil posterior sudah ada
            $sql_check_posterior = "SELECT * FROM posterior WHERE karyawan_id = {$karyawan['id']}";
            $result_check_posterior = $conn->query($sql_check_posterior);

            if ($result_check_posterior->num_rows == 0) {
                // Simpan hasil posterior
                $sql_posterior = "INSERT INTO posterior (karyawan_id, hasil_prediksi) VALUES ({$karyawan['id']}, '$hasil_klasifikasi')";
                if (!$conn->query($sql_posterior)) {
                    die("Error inserting posterior: " . $conn->error);
                }
            }
        }
        ...
        // Simpan likelihood ke dalam tabel likelihood
        foreach ($likelihood as $criteria_id => $likelihood_values) {
            foreach ($likelihood_values as $kelas => $values) {
                foreach ($values as $value => $count) {
                    $probabilitas = $count / $total_data;
                    $sql_likelihood_insert = "INSERT INTO likelihood (criteria_id, kelas, nilai, likelihood_naik, likelihood_tidak_naik) VALUES ($criteria_id, '$kelas', '$value', $count, $probabilitas)";
                    if (!$conn->query($sql_likelihood_insert)) {
                        die("Error inserting likelihood: " . $conn->error);
                    }
                }
            }
        }
    ...
    hitungNaiveBayes($conn, $karyawan_data, $criteria, $prior, $likelihood, $total_data);
...
$sql_results = "SELECT td.status as actual, hk.hasil_prediksi as predicted 
               FROM posterior hk 
    ...
    <div class="card-body px-0 pb-2">
        <div class="table-responsive p-0">
            <table class="table table-hover align-items-center mb-0">
                ...
                <tbody>
                    <?php
                    // Query untuk mengambil data likelihood dengan join ke tabel criteria dan sub_criteria
                    $sql_likelihood = "
                        SELECT 
                            l.nilai, 
                            l.jumlah, 
                        ...
                        JOIN sub_criteria sc ON sc.criteria_id = c.id AND sc.value = l.nilai";
                    ...
                    if ($result_likelihood && $result_likelihood->num_rows > 0) {
                        while ($row = $result_likelihood->fetch_assoc()) {
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <?php echo number_format($row['probabilitas'], 4); ?>
                                </td>
                            </tr>
                            <?php
                        }
                ...
                <div class="table-responsive p-0">
                    <table class="table table-hover align-items-center mb-0">
                        ...
                        <tbody>
                            ...
                            $sql_posterior = "SELECT k.nama, p.hasil_prediksi 
                                              FROM posterior p 
                            ...
                            $result_posterior = $conn->query($sql_posterior);
                                                        ...
                                                        if ($result_posterior && $result_posterior->num_rows > 0) {
                                                          ...
                                                          ?>
                                                          <tr>
                                                              <td colspan="2" class="text-center">Tidak ada data hasil klasifikasi</td>
                                                          </tr>
                                                          <?php
                                                      }
                                                      ?>
                ...
                <div class="card-body px-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table table-hover align-items-center mb-0">
                            ...
                            <tbody>
                                <?php
                                $sql_hasil = "SELECT k.*, h.status FROM karyawan k LEFT JOIN hasil_klasifikasi h ON k.id = h.karyawan_id";
...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
 ...
 // Tentukan hasil klasifikasi
            ...
            // Simpan hasil klasifikasi ke dalam tabel hasil_klasifikasi (jika belum ada)
            // $sql_check = "SELECT * FROM hasil_klasifikasi WHERE karyawan_id = {$karyawan['id']}";
            // $result_check = $conn->query($sql_check);
            ...
            // // Simpan hasil posterior ke dalam tabel posterior (jika belum ada)
            // $sql_check_posterior = "SELECT * FROM posterior WHERE karyawan_id = {$karyawan['id']}";
            // $result_check_posterior = $conn->query($sql_check_posterior);
    
            // if ($result_check_posterior->num_rows == 0) {
            //     // Ambil nilai likelihood dari tabel likelihood
            //     $sql_likelihood = "
            //         SELECT criteria_id, likelihood_naik, likelihood_tidak_naik
            //         FROM likelihood
            ...
            //     if ($result_likelihood->num_rows > 0) {
            //         $posterior_naik = 1.0;
            //         $posterior_tidak_naik = 1.0;
    
            //         // Kalkulasi nilai posterior
            //         while ($row_likelihood = $result_likelihood->fetch_assoc()) {
            ...
            //             VALUES ({$karyawan['id']}, $posterior_naik, $posterior_tidak_naik)
            //         ";
            //         if (!$conn->query($sql_save_posterior)) {
            //             die("Error inserting posterior: " . $conn->error);
            //         }
            //     } else {
            //         // Jika tidak ada data likelihood yang sesuai
            //         die("No likelihood data found for karyawan_id: {$karyawan['id']}");
            //     }
            // }