...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['nama'])) {
    ...
    try {
        ...
        foreach ($kriteria_values as $kriteria_id => $value) {
            $sql = "INSERT INTO karyawan_kriteria (karyawan_id, criteria_id, value) VALUES (?, ?, ?)";
            $stmt = $conn->prepare($sql);
            if ($stmt === false) {
                die('Prepare failed: ' . $conn->error);
            }
            $stmt->bind_param("iii", $karyawan_id, $kriteria_id, $value);
            if (!$stmt->execute()) {
                throw new Exception($stmt->error);
            }
        }
...
<div class="modal fade" id="addDataModal" tabindex="-1" aria-labelledby="addDataModalLabel" aria-hidden="true">
                    ...
                    foreach ($criteria as $c) {
                        ...
                        if (isset($sub_criteria[$c['id']])) {
                            foreach ($sub_criteria[$c['id']] as $sub) {
                                echo "<option value='{$sub['value']}'>{$sub['name']}</option>";
                            }
                        }
                        echo "</select>";
                        echo "</div>";
 ...
 <!-- Modal Edit Data -->
<?php if ($result->num_rows > 0): ?>
    <?php $result->data_seek(0); // Mengatur kursor ke baris pertama untuk mengulang hasil ?>
    <?php while ($row = $result->fetch_assoc()): ?>
        <div class="modal fade" id="editEmployeeModal<?php echo $row['id']; ?>" tabindex="-1" aria-labelledby="editEmployeeModalLabel<?php echo $row['id']; ?>" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editEmployeeModalLabel<?php echo $row['id']; ?>">Edit Data Pegawai</h5>
                    ...
                    <div class="modal-body">
                        <!-- Form Edit Data -->
                        <form id="editEmployeeForm<?php echo $row['id']; ?>" action="edit_pegawai.php" method="POST">
                            ...
                            <!-- Loop untuk Kriteria dan Sub-Kriteria -->
                          <?php foreach ($criteria as $c): ?>
                              <label for="kriteria_<?php echo $c['id']; ?>" class="form-label"><?php echo $c['name']; ?>:</label>
                              <div class="input-group input-group-outline mb-3">
                                  <select class="form-control" id="kriteria_<?php echo $c['id']; ?>" name="kriteria_<?php echo $c['id']; ?>" required>
                                      <?php foreach ($sub_criteria[$c['id']] as $sub): ?>
                                          <?php
                                          // Tentukan nilai yang akan dipilih (selected)
                                          $selected = ($sub['value'] == $criteria_value[$c['id']]) ? 'selected' : '';
                                          ?>
                                          <option value="<?php echo $sub['value']; ?>" <?php echo $selected; ?>><?php echo $sub['name']; ?></option>
                                      <?php endforeach; ?>
                                  </select>
                              </div>
                          <?php endforeach; ?> 
